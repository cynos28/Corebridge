...
const TicketsPage = () => {
  ...
  const handleDeleteTicket = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await Swal.fire('Deleted!', 'The ticket has been successfully deleted.', 'success');
      } catch (error) {
  ...
  const generateTicketPDF = (ticket: Ticket) => {
    ...
    let y = 60;
    if (ticket._id) { doc.text(`Ticket ID: ${ticket._id}`, 20, y); y += 10; }
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <input
            ...
            className="flex-grow md:flex-none px-4 py-2 border border-gray-300 rounded-lg"
          />
          <button
...
const TicketForm: React.FC<TicketFormProps> = ({ onSubmit, onCancel, initialData }) => {
  ...
  const validate = () => {
    ...
    if (!/^[A-Za-z\s]+$/.test(studentName)) {
      newErrors.studentName = "Only letters and spaces are allowed.";
    }
    ...
    <form onSubmit={handleSubmit} className="w-full">
      <div className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <button
            type="submit"
            className="w-full bg-cbYellow text-white py-2 px-4 rounded-lg"
          >
